...
class ClassCounter:
    ...
    def dump_new_class(self, project_name):
        if len(self.new_class_set) != 0:
            with open(f"{project_name}NewClasses.csv", "w", encoding="utf-8") as file:
                for new_class in self.new_class_set:
                    file.write(str(new_class) + "\n")

    def dump_sub_class_rel(self, project_name):
        if len(self.new_class_inherit_set) != 0:
            with open(f"{project_name}NewClassInherit.csv", "w", encoding="utf-8") as file:
...
if __name__ == '__main__':
    ...
    classCounter.dump_new_class(project_name)
    classCounter.dump_sub_class_rel(project_name)
